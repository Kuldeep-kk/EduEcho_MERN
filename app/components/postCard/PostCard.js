...
const PostCard = memo(({ data, username, userProfileUrl, post, postImages, createdAt }) => {
    ...
    const handleLikes = async () => {
        const result = await AddLike(data._id, context.user?._id, context.user?.name, context.user?.secure_url, context.user?.designation);
        await context.loadPost();
        console.log(result.status ? "You liked" : "It's already liked by you", result);
    };

    const handleSave = async () => {
        const result = await AddSavedUser(data._id, context.user?._id, context.user?.name, context.user?.secure_url);
        await context.loadPost();
