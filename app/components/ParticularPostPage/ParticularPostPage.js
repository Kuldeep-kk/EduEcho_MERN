...
import {commentPost, getAllPost, likePost} from "@/app/services/postService";
...
const ParticularPostPage = () => {
    ...
    const handleComment=async ()=>{
        const result=await commentPost(context.postDialogData._id,context.user?._id,context.user?.name,context.user?.secure_url,myComment);
    ...
    const handleLikes= async()=>{
        const result= await likePost(context.postDialogData._id,context.user?._id,context.user?.name,context.user?.secure_url);
